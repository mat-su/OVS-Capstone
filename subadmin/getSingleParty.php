...
extract($_POST);
$partylists = $conn->prepare("SELECT p.id AS id, p.pname AS partylist FROM tbl_partylist p LEFT JOIN tbl_stud_orgs o ON p.p_orgid = o.org_id WHERE p.p_orgid = {$sa_orgid} = :orgid AND p.id = :id");
